...
| codecoachai-system | 系统服务 | 是 | 管理端基础能力、系统配置 |
...
V1 如果开发压力较大，菜单和权限可进一步简化，优先保证用户角色和管理端访问控制。
...
3. `/questions/**` 和 `/admin/questions/**` 转发到 question-service。
4. `/resumes/**` 转发到 resume-service。
5. `/interviews/**` 转发到 interview-service。
6. `/ai/**` 和 `/admin/ai/**` 转发到 ai-service。
7. `/admin/system/**` 转发到 system-service。
...
1. 实现业务流程。
2. 编排 mapper、FeignClient 和 AI 服务调用。
3. 控制事务。
4. 处理状态流转。
5. 处理业务异常。
...
1. 当前服务配置类。
2. MyBatis、Redis、Feign、Swagger、AI 等配置。
...
2. 跨服务获取数据必须通过 OpenFeign 调用服务接口。
...
3. 面试报告生成失败时，interview-service 将会话状态置为 `FAILED` 或报告生成失败状态。
4. 不引入 Seata。
5. 不引入消息队列补偿机制。
...
5. 增强 ai-service，支持简历解析、简历优化、题目生成、学习计划、题库去重判断。
6. 增强 system-service，支持行业模板、Prompt 版本管理。
7. 增加 SSE 流式输出能力。
...
8. System 负责管理端基础能力和系统配置。
9. Common 负责统一返回、异常、安全、MyBatis、Redis、Feign、AI 通用对象和日志基础能力。
